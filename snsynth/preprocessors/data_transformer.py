...
class BaseTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        ...
        ohe = OneHotEncoder()
        ohe.fit(pd.DataFrame(raw_column_data.dropna()), column_name)
    ...
    def fit(
        ...
        for column_name in raw_data.columns:
            raw_column_data = raw_data[column_name]
    ...
    def _transform_continuous(self, column_transform_info, raw_column_data):
        ...
        return [raw_column_data.values]

    def _transform_discrete(self, column_transform_info, raw_column_data):
        ohe = column_transform_info.transform
        raw_column_data_nomissing = raw_column_data.dropna()
        raw_column_data_nomissing = ohe.transform(raw_column_data_nomissing)
        attrwmissing = raw_column_data.astype('str').to_numpy(na_value = np.nan) #convert to numpy with nan elsewhere
        attrwmissing = np.empty((attrwmissing.shape[0], raw_column_data_nomissing.shape[1]))# create np array with nan
        attrwmissing[:] = np.nan
        mask = (~pd.isnull(raw_column_data)).to_numpy().reshape(-1)
        attrwmissing[np.where(mask)] = raw_column_data_nomissing
        
        return [attrwmissing]
        # return [ohe.transform(pd.DataFrame(raw_column_data, columns=ohe.columns))]
    ...
    def transform(self, raw_data):
        ...
        for column_transform_info in self._column_transform_info_list:
            column_data = raw_data[[column_transform_info.column_name]]
    ...
    def _inverse_transform_discrete(self, column_transform_info, column_data):
        ...
        return ohe.reverse_transform(pd.DataFrame(column_data, columns=ohe.output_columns))
