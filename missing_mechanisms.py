...
def MAR_mask(X, p, p_obs):
    ...
    coeffs = pick_coeffs(X, idxs_obs, idxs_nas).double()
...
def MNAR_mask_logistic(X, p, p_params =.3, exclude_inputs=True):
    ...
    ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, idxs_params, idxs_nas).double()
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X[:, idxs_params], coeffs.double(), p)
...
def MNAR_self_mask_logistic(X, p):
    ...
    ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, self_mask=True).double()
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X, coeffs.double(), p, self_mask=True)

    ps = torch.sigmoid(X * coeffs.double() + intercepts)
